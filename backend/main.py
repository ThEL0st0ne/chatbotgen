...
BASE_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))  # Get script directory
TEMPLATE_DIR = os.path.join(BASE_DIR, "backend\\template")

HTML_FILE = os.path.join(TEMPLATE_DIR, "backend\\index.html")
CSS_FILE = os.path.join(TEMPLATE_DIR, "backend\\style.css")
JS_FILE = os.path.join(TEMPLATE_DIR, "backend\\script.js")

FOLDER_PATH = os.path.join(BASE_DIR, "frontend\\bots")  # Ensure absolute path
...
# Dynamic route to serve file content
...
async def save_gradient(input_data: chatgradient):
    ...
    return {"message": "Input 2 saved", "data": chat_gradient}
